...
MG.time_format = function(utc, specifier) {
  ...
  if (specifier.indexOf("%b") != -1)
    specifier = specifier.replace("%b %d", "%m/%d");
  // add end.
...
// modify start 2017/10/31 日付書式を変更 %Y/%m/%dに。
//%b - abbreviated month name.*
...
function mg_get_rollover_time_format(args) {
  ...
  switch (args.processed.x_time_frame) {
    case 'millis':
      ...
      fmt = MG.time_format(args.utc_time, '%Y/%m/%d %H:%M:%S.%L');
      break;
    case 'seconds':
      //fmt = MG.time_format(args.utc_time, '%b %e, %Y  %H:%M:%S');
      fmt = MG.time_format(args.utc_time, '%Y/%m/%d %H:%M:%S');
      break;
    case 'less-than-a-day':
      //fmt = MG.time_format(args.utc_time, '%b %e, %Y  %I:%M%p');
      fmt = MG.time_format(args.utc_time, '%Y/%m/%d %H:%M');
      break;
    case 'four-days':
      //fmt = MG.time_format(args.utc_time, '%b %e, %Y  %I:%M%p');
      fmt = MG.time_format(args.utc_time, '%Y/%m/%d %H:%M');
    ...
    default:
      ...
      fmt = MG.time_format(args.utc_time, '%Y/%m/%d');
  }
  return fmt;
}
// modify end.
...
function mg_place_marker_text(gm, args) {
  gm.selectAll('.mg-markers')
    ...
    .append('text')
      ...
      //.attr('text-anchor', 'middle') //modify 2017/9/27 toda cssの設定を引き継ぐために切っています。
...
function process_line(args) {
  ...
  if ((args.missing_is_zero || args.missing_is_hidden) && args.chart_type === 'line' && is_time_series) {
    for (var i = 0; i < args.data.length; i++) {
      ...
      // commentout データを間引かない 2017/7/11 args.data[i] = processed_data;
    }
  }
